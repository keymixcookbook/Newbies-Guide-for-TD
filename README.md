# Nuke Modding Guide for Newbies

More of a learning Repository for Nuke Python module

### Table of Content (Respository)

- [Modding with Python](./_python)
- [Modding with Expression/TCL](./_tcl)
- [~~Modding with PySide~~](###)
- [~~Modding with Blink Script~~](###)

###### > Online References and Guides <
- [Nuke Python API (Bible)](https://learn.foundry.com/nuke/developers/70/pythonreference/)
- [Nuke Developer's Guide ('Quick' start)](https://learn.foundry.com/nuke/developers/80/pythondevguide/index.html)
- [Python 101 (Legible Learning Site)](https://python101.pythonlibrary.org/index.html)

###### > Module APIs <
- [**Nuke** (functions used in Nuke)](https://learn.foundry.com/nuke/developers/70/pythonreference/)
- [**OS** (for accessing system files)](https://docs.python.org/2.7/contents.html)
- [**Blinkscript** (to use with Blinkscript Node)](https://learn.foundry.com/nuke/developers/11.2/BlinkKernels/index.html)
